...
# %%
model_names = {
    "32b": "Qwen3-4b",  # NOTE: change to the name of your 32b large model, e.g. "org/model-32b"
    "4b": "DeepSeek-R1-Distill-Qwen-1.5B",  # NOTE: change to the name of your 4b small model
}
...
def get_first_user_msg(problem, options=None):
    if options == "aime" or options == "math":
        ...
        system_prompt += f"Problem: {problem['question']}\n\n"
        return system_prompt
    else:
...
def glimprouter(
    ...
    try:
        ...
        t1 = time.perf_counter()
        base_model_step, finished, num_output_tokens_base, response = generate_answer(
